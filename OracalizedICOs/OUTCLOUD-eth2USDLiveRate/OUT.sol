...
contract OutCloud is ERC20, usingOraclize {
    ...
    address public Fundswallet; // address to receive ether from smart contract
    ...
    function buyTokens() public payable {
        sen = msg.sender;
        weiAmount = msg.value;
        _preValidatePurchase(sen, weiAmount); // checks minimum requirements
        updatePrice();
    }
    
    function _pro (address _beneficiary, uint256 weiAmount) internal {
        _preValidatePurchase(_beneficiary, weiAmount); // checks minimum requirements
    ...
    function _calculateMinimumprice() internal view returns (uint256){
        ...
        if(ETHUSD == 0){
         minimumWeisRequired= (minContribution.mul(ETH_DECIMALS)).div(300);
        }
        else{
         minimumWeisRequired= (minContribution.mul(ETH_DECIMALS)).div(ETHUSD);
        }
    ...
    function _setRate() internal{
        uint256 ETHUSD = parseInt(ETHUSDPrice,0);
        ...
        if (stage == Stages.PREICO && now <= preico_enddate){
            ...
            // pre-ico is open
            currentTokenPriceFactor = (block.timestamp.sub(preico_startdate)).div(900); // 1 week time period in seconds
    ...
    function _forwardFunds() internal {
        Fundswallet.transfer(weiAmount.div(2));
    }
    ...
    function setEthmainAddress(address newEthfundaddress) external onlyOwner{
        Fundswallet = newEthfundaddress;
    }
 
    function start_PREICO() external onlyOwner atStage(Stages.NOTSTARTED){
        stage = Stages.PREICO;
        stopped = false;
        // balances[this] = 300000000 * 10 ** 18 ; //300 Million in pre-ICO
    ...
    function start_ICO() external onlyOwner atStage(Stages.PREICO){
        stage = Stages.ICO;
        stopped = false;
        // balances[this] = balances[this].add(TotalICOSupply) ; //400 Million in ICO
        ico_startdate = now;
        //  ico_enddate = now + 28 days; //time period for ICO = 4 weeks
        // Transfer(0, address(this), TotalICOSupply);

...
contract OBLContract is ERC20Interface{
    ...
    function totalSupply() public constant returns (uint){
       return 20000000000 * 10**uint(decimals); //20 billion
    }
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount, uint256 tokens) view internal {
        ...
        require(balances[_beneficiary] + tokens  <= 650 * 10**uint(decimals));
    }
	// ------------------------------------------------------------------------
